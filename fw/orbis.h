...
class Orbis {
 public:
  Orbis(const aux::UartEncoder::Config& config,
         Stm32G4DmaUart* uart,
  ...
  void ProcessQuery(aux::UartEncoder::Status* status) MOTEUS_CCM_ATTRIBUTE {
    ...
    status->active = status->orbis_err;
  }
 ...
 private:
  ...
  // The "detailed" reply has a header byte, 2 bytes of position, and
  // 1 byte of status.
  //
  // We have 3 extra bytes so that we could eventually re-synchronize.
  uint8_t buffer_[4 + kResyncBytes] = {};
};
